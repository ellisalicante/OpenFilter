...
import sys
sys.path.append("../")
sys.path.append("/home/bill/code/face_recognition/magface")
...
def load_feat_pair(feat_path, pair_path):
    ...
    is_pair = np.load(pair_path)
    is_pair = is_pair.tolist()
...
def perform_1v1_eval(args):
    ...
    # Make sure to choose the same folder as in python argument
    # Also remember to change L71 in utils_callbacks for what data to load (orig, gauss, etc.)
    #np.save('/mnt/c/Users/Bill/Desktop/fairfaces_val_pixelation/02/magface_dist.npy', dist)
    #print(np.mean(dist))
...
def main():
    ...
    logging.info('Running on: %s' % (args.feat_list.split('/')[-1].split('.')[0]))
    tpr, fpr, accuracy, norm_dist = perform_1v1_eval(args)
    logging.info('Accuracy: {}+-{}'.format(np.mean(accuracy), np.std(accuracy)))
    logging.info('Sum of Euclidean dists: {}'.format(np.sum(norm_dist)))
